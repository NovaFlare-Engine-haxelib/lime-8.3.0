...
namespace lime { 
    ...
    void RenderThread::Flip(bool forceWait) {
        ...
        // AUTO-MOD: Strict buffering limit for low latency
        // If we have more than 1 frame pending, drop the current one (Mailbox style)
        if (!forceWait && activePendingFrames > 1) {
            currentFrame->clear();
            return;
        }
